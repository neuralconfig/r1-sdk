#!/usr/bin/env python3
"""
Test script for authentication and listing venues in the RUCKUS One SDK.
"""
import os
import sys
import logging
...
def main():
    """Main function to test authentication and venue listing."""
    ...
    # Initialize venues module
    venues_module = Venues(client)
    
    # Test authentication by listing venues
    print("Testing authentication by listing venues...")
    try:
        venues_result = venues_module.list()
        venues_list = venues_result.get('data', [])
        print(f"Authentication successful! Found {len(venues_list)} venues:")
        for venue in venues_list:
            print(f"- {venue.get('name', 'Unknown')} (ID: {venue.get('id', 'Unknown')})")
        
        # Print the first venue details
        if venues_list:
            print("\nDetails of the first venue:")
            pprint(venues_list[0])
            
    except Exception as e:
        print(f"Error during authentication or venue listing: {e}")
        raise
