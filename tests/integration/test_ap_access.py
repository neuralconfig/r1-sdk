#!/usr/bin/env python3
"""
Test script for accessing AP information in the RUCKUS One SDK.
"""
import os
import sys
import logging
...
def main():
    """Main function to test AP access."""
    ...
    # Get APs in the venue
    print("\nGetting access points in the venue...")
    try:
        # Use the general AP query endpoint with simple parameters
        query_data = {
            "pageSize": 100,
            "page": 0
        }
        ...
        print(f"Found {len(aps_list)} access points:")
        for ap in aps_list:
            print(f"- {ap.get('name', 'Unknown')} (Model: {ap.get('model', 'Unknown')}, MAC: {ap.get('apMac', 'Unknown')})")
        
        # Print the first AP details
        if aps_list:
            print("\nDetails of the first access point:")
            pprint(aps_list[0])
